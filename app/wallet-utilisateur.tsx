...
import SecureScreen from "./components/SecureScreen"; // ✅ SÉCURITÉ
...
export default function WalletUtilisateurRHAZN() {
  ...
  return (
    <SecureScreen scope="Wallet"> {/* ✅ AUTH BIOMÉTRIQUE + PIN */}
      <View style={styles.container} {...panResponder.panHandlers}>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 150, paddingTop: 150 }}>
          ...
          <View style={styles.grid}>
            <MenuCard
              icon={<Feather name="send" size={26} color="#FFD700" />}
              label="Envoyer TAN"
              onPress={() => router.push("/send-tan-user")}
            />

            <MenuCard
              icon={<MaterialIcons name="payments" size={26} color="#FF9800" />}
              label="Paiement TAN (Agent)"
              onPress={() => router.push("/withdraw-tan")}
            />

            <MenuCard
              icon={<Ionicons name="location" size={26} color="#4ade80" />}
              label="Trouver Agent RZ"
          ...
          <View style={styles.rulesBox}>
            ...
            <Text style={styles.rule}>• 1 ACSET = 250 TAN (publication)</Text>
            <Text style={styles.rule}>• 1 SUSPENTZ = 125 TAN</Text>
            <Text style={styles.rule}>• Max 5 SUSPENTZ / semaine</Text>
            <Text style={styles.rule}>• Transfert TAN min : 500 | max : 100 000</Text>
            <Text style={styles.rule}>• Paiement Agent : min 5 000 | max 500 000 TAN</Text>
