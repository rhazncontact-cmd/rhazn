// ✅ Flux du Mérite — Intro supprimée
...
// ✅ 3 PACTS test — tous "isMine" pour bouton inviter
const videos = [
...
// ✅ Fonctions temps / Sabbat (inchangées)
function nowInHaiti() { const now = new Date(); const utc = now.getTime() + now.getTimezoneOffset() * 60000; return new Date(utc + HAITI_UTC_OFFSET_HOURS * 3600 * 1000); }
function isShabbat(d) { const day = d.getUTCDay(), h = d.getUTCHours(), m = d.getUTCMinutes(); return (day === 5 && h >= 18) || (day === 6 && h < 17) || (day === 6 && h === 17 && m === 0); }
function isFriday1750(d) { return d.getUTCDay() === 5 && d.getUTCHours() === 17 && d.getUTCMinutes() === 50; }
function cycleDay(d) { return ((d.getUTCDate() - 1) % 30) + 1; }
const two = n => (n < 10 ? `0${n}` : `${n}`);

export default function FluxDuMerite() {
  ...
  const copyLink = async () => { await Clipboard.setStringAsync(`rhazn://pact/${shareFor}`); Alert.alert("Lien copié ✅"); };
      ...
      <Modal visible={shareOpen} transparent animationType="fade">
        <View style={styles.modalWrap}>
          <View style={styles.modalCard}>
            ...
            {!!shareFor && <QRCode value={`rhazn://pact/${shareFor}`} size={180} backgroundColor="transparent" color="#D4AF37" />}

            <Text selectable style={styles.linkTxt}>{`rhazn://pact/${shareFor ?? ""}`}</Text>
