...
export default function PinVerifyModal({ visible, onSuccess, onCancel, showManageLink = true }: Props) {
  ...
  useEffect(() => {
    if (visible) {
      setPin("");
      setTries(0);
      rzSecure.get(PIN_KEY).then((v) => setHasPin(!!v));
    }
  }, [visible]);
  ...
  const verifyPin = async (attempt: string) => {
    ...
    if (attempt === saved) {
      ...
      setTries(newTries);
      if (newTries >= 5) {
        setAlertMsg("Trop de tentatives incorrectes. Vérifiez votre PIN dans les paramètres.");
      } else {
        setAlertMsg(`PIN incorrect. ${5 - newTries} tentative${5 - newTries > 1 ? "s" : ""} restante${5 - newTries > 1 ? "s" : ""}.`);
      }
      setAlertV(true);
    }
  };
