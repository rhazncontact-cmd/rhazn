// app/rz-admin-governance/admin-finance/ecoformules.tsx
...
/* 🍎 RHAZN — EcoFormules (Règles Macro-Économiques)
   ⚠️ Ce fichier NE gère PAS l’activation des catégories.
   🔒 Le verrou est côté DB via assert_ecoformule_active().
*/
...
export default function RZAdminEcoFormules() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (error || !data) {
        setError("Impossible de charger les règles économiques globales.");
      } else {
  ...
  const save = async () => {
    ...
    if (error) {
      ...
      Alert.alert("Succès", "Règles macro-économiques mises à jour.");
    }
  };
    ...
    return (
      <View style={styles.center}>
        <Text style={{ color: "#f87171", textAlign: "center" }}>
          Aucune règle économique trouvée.
        </Text>
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Économie globale (EcoFormules)</Text>
        <View style={{ width: 26 }} />
      </View>
      ...
      <Section title="Services utilisateur">
        <Field label="Prix photo de profil (TAN)" value={formulas.profile_photo_change_price}
  ...
  return (
    <View style={styles.infoBox}>
      <Text style={styles.infoText}>
        Ces règles définissent l’économie globale (TAN, ACSET, commissions).
        {"\n\n"}
        ⚠️ L’activation des catégories se fait désormais séparément
        via les EcoFormules par catégorie.
      </Text>
    </View>
  );
}
